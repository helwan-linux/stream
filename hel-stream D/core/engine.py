...
class UniversalStreamEngine:
    ...
    def get_stream_url(self, url, requested_quality="best"):
        """تعديل الدالة لتدعم اختيار الجودة المطلوبة"""
        try:
            # لو المستخدم اختار Auto أو Best هنمشي بالنظام القديم
            # لو اختار جودة محددة هنحاول نجيبها
            fmt = f"bestvideo[height<={requested_quality[:-1]}]+bestaudio/best" if requested_quality != "Auto" else "best"
            
            with yt_dlp.YoutubeDL({'format': fmt, 'quiet': True, 'nocheckcertificate': True}) as ydl:
                info = ydl.extract_info(url, download=False)
