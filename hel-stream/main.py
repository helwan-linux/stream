...
class HelwanStreamApp(MainWindow):
	...
	def display_results(self, results):
		...
		for res in results:
			...
			# --- تعديل ذكي لقراءة المصدر من المحرك ---
			raw_source = res.get('source', 'WEB')
			# تحويل الاختصارات لأسماء كاملة وشيك
			if raw_source.lower() == 'youtube':
				platform_tag = "[YOUTUBE]"
			elif raw_source.lower() == 'soundcloud':
				platform_tag = "[SOUNDCLOUD]"
			else:
				platform_tag = f"[{raw_source.upper()}]"
			
			# دمج المصدر مع اسم المرفوع (Uploader) عشان العنوان يفضل نظيف
			uploader_info = f"{platform_tag} | {res['uploader']}"
			# ---------------------------------------

			card = VideoCard(res['title'], uploader_info, res['duration'], res['thumbnail'])
