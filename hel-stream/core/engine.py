...
class UniversalStreamEngine(QObject): # جعل الكلاس يدعم السيجنالز
	...
	def get_stream_url(self, url: str, requested_quality: str = "best") -> str:
		...
		# 1. منطق تحديد الصيغة (Format Logic)
		if requested_quality == "Audio Only":
			# طلب أفضل صوت فقط
			fmt = "bestaudio/best"
		elif requested_quality != "best" and requested_quality != "Auto":
			# تنظيف النص (مثلاً 240p تصبح 240)
			clean_res = requested_quality.replace('p', '')
			# طلب فيديو لا يتعدى الطول المطلوب + أفضل صوت، أو أفضل ملف مدمج لا يتعدى الطول
			fmt = f"bestvideo[height<={clean_res}]+bestaudio/best[height<={clean_res}]/best"
		else:
			fmt = "best"
		...
		for i, ua in enumerate(user_agents):
			try:
				...
				with yt_dlp.YoutubeDL(opts) as ydl:
					info = ydl.extract_info(url, download=False)
					# ملاحظة: في حالة طلب فيديو وصوت منفصلين، yt-dlp قد لا يعطي رابطاً واحداً في 'url'
					# بل يعطي الروابط داخل 'formats'. لكن للمشغلات مثل mpv، يفضل رابط واحد.
					return url, fmt

			except Exception:
				# إذا فشل الجودة المطلوبة، جرب الإعداد الافتراضي قبل الانتقال للوكيل التالي
				fmt = "best"
				continue
