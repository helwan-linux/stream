...
# --- جديد: Worker تجهيز البث (Silent Recovery Support) ---
# هذا الـ Worker يضمن عدم تجمد الواجهة أثناء محاولة المحرك تجاوز الحظر أو تغيير الجودة
class StreamWorker(QThread):
    stream_ready = pyqtSignal(str, str) # يرسل رابط البث النهائي والجودة
    status_msg = pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            # استدعاء المحرك المطور الذي يحتوي على محاولات التكرار وتغيير الـ User-Agent
...
class HelwanStreamApp(MainWindow):
    ...
    # الدالة الجديدة التي تفتح المشغل بعد تجهيز الرابط
    def launch_mpv_player(self, stream_url, quality):
        ...
        if player_bin:
            ...
            # تشغيل المشغل بالرابط النهائي
            self.current_player_process.start(player_bin, [stream_url])
            self.status_label.setText(f"Streaming in {quality} | Enjoy")
